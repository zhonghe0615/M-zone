...
if ($_GET["delete"] != null){
	...
	$stmt = $dbh->prepare("Delete FROM taste where username = ? and typeid= ?");
...
if ($_GET["add"] != null){
	...
	$stmt = $dbh->prepare("INSERT INTO taste(username,typeid) VALUES(?,?)");
	$stmt->bindParam(1, $_SESSION["username"]);
...
<body class="templatemo-bg-gray">
	<h1 class="margin-bottom-15">Edit Your Taste</h1>
	<div class="container">
		<div class="col-md-12">
			<form
				class="form-horizontal templatemo-create-account templatemo-container"
				role="form" action="./edittaste.php" method="post">
				<div class="form-inner">
					<div class="form-group">
						<div class="col-md-6">
							<label for="username" class="control-label">Your Taste</label></br>
							<?php 
							$stmt = $dbh->prepare("SELECT * FROM taste, type where taste.username = ? and type.typeid = taste.typeid");
							$stmt->bindParam(1, $_SESSION["username"]);
							$stmt->execute();
							...
							<img src="./images/error.gif"
								onclick="location='edittaste.php?delete=<?=$row["typename"] ?>'"></img>
							<?php 
							echo "</br>";
							}
							?>
				...
				if ($_POST["typename"]!=null){
					...
					while ($row = $stmt->fetch()){
						...
						<img src="./images/35.png" style="width:18px; height:18px;"
							onclick="location='edittaste.php?add=<?=$row["typename"] ?>'"></img>
						<?php 
						echo "</br>";
					}
				}
