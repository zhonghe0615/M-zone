...
if ($_GET["delete"] != null){
	...
	$stmt = $dbh->prepare("Delete FROM ctype where concertID = ? and typeid= ?");
...
if ($_GET["add"] != null){
	...
	$stmt = $dbh->prepare("INSERT INTO ctype(concertID,typeid) VALUES(?,?)");
	$stmt->bindParam(1, $_GET[concertID]);
...
$stmt = $dbh->prepare("SELECT * FROM user where concertID = ?");
$stmt->bindParam(1, $_GET[concertID]);
$stmt->execute();
$row = $stmt->fetch();
...
<body class="templatemo-bg-gray">
	<h1 class="margin-bottom-15">Edit Your Type</h1>
	<div class="container">
		<div class="col-md-12">
			<form
				class="form-horizontal templatemo-create-account templatemo-container"
				role="form" action="./editctype.php?concertID=<?=$_GET["concertID"] ?>" method="post">
				<div class="form-inner">
					<div class="form-group">
						<div class="col-md-6">
							<label for="username" class="control-label">Concert type</label></br>
							<?php 
							$stmt = $dbh->prepare("SELECT * FROM ctype, type where ctype.concertID = ? and type.typeid = ctype.typeid");
							$stmt->bindParam(1, $_GET["concertID"]);
							$stmt->execute();
							...
							<img src="./images/error.gif"
								onclick="location='editctype.php?delete=<?=$row["typename"] ?>&concertID=<?=$_GET["concertID"] ?>'"></img>
							<?php 
							echo "</br>";
							}
							?>
				...
				if ($_POST["typename"]!=null){
					...
					while ($row = $stmt->fetch()){
						...
						<img src="./images/35.png" style="width:18px; height:18px;"
							onclick="location='editctype.php?add=<?=$row["typename"] ?>&concertID=<?=$_GET["concertID"] ?>'"></img>
						<?php 
						echo "</br>";
					}
				}
