...
if (is_uploaded_file($_FILES['upfile']['tmp_name'])){
	...
	if($ok && $error=='0'){
		...
		$stmt = $dbh->prepare("Update artist set aimgurl= ? where username = ?");
		$name='album/'.$name;
		$stmt->bindParam(1, $name);
		$stmt->bindParam(2, $_SESSION["username"]);
		$stmt->execute();
		echo "<script language=\"javascript\">location.href='personal2.php';</script>";
	}
}
    ...
    <div id="templatemo_main">
		<div class="content_wrapper content_mb_60">
            <div class="col_2">
                ...
                <a href="" class="more" onClick="window.open('editartist.php','','height=550,width=620,scrollbars=yes,status =yes')">Edit</a>
            </div>        
            <div class="col_2 no_margin_right">
                <h2>Face</h2>
                <?php if($row[aimgurl] != null){ ?>
                <p><img src="<?=$row[aimgurl] ?>"></img> </p>
		 		<?php 
				}else{
				?>
        ...
        <div class="content_wrapper content_mb_60">
            <div class="col_4">
                ...
                <?php 
                $stmt = $dbh->prepare("SELECT * FROM play, type where play.username = ? and type.typeid = play.typeid limit 0,3");
                $stmt->bindParam(1, $_SESSION["username"]);
                $stmt->execute();
                ...
                <a href="" class="more" onClick="window.open('editplay.php','','height=550,width=620,scrollbars=yes,status =yes')">More</a>
            </div>  
