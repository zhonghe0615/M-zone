    ...
    <div id="templatemo_main">
        <div class="content_wrapper content_mb_60">
            <div class="col_34 no_margin_right">
                <h3>Verify Artist</h3>
                <form action="./Service/verify_cfg.php?action=search" method="post">
                <p>
                Artist Name: <input type="text" name="artistname" id="artistname"></input> &nbsp;&nbsp;
                Verification: 
		        ...
		        <input type="radio" name="optionsRadios" id="optionsRadios3" value="2" checked="checked">All
                </p>
               	<input name="submit" type="image" src="./images/search.jpg"
				align="absmiddle" style="margin-top: 8px;"
            ...
            <th width="300">WebLink</th>
            <th width="150">Verification</th>
            <th width="100">Permission</th>
            <th width="100">Action</th>
            </tr>
            <?php 
            if ($_GET[artistname] != null || $_GET[veri] != null){
            	$artistname ="%" . $_GET[artistname]. "%";
            	$veri = $_GET[veri];
            	$stmt;
            	if ($veri != 2) {
            		$stmt = $dbh->prepare("SELECT * FROM artist where name like ? and verification = ?");
            		$stmt->bindParam(1, $artistname);
            		$stmt->bindParam(2, $veri);
            		$stmt->execute();
            	}else{
            		$stmt = $dbh->prepare("SELECT * FROM artist where name like ?");
            		$stmt->bindParam(1, $artistname);
            		$stmt->execute();
            	}
            ...
            ?>
            <img src="images/2.png" onclick="location='./Service/manage_cfg.php?yes=<?=$row[username] ?>'" style="width: 20px;"></img>
            <?php
            }else{
           	?>
            <img src="images/5.png" onclick="location='./Service/manage_cfg.php?no=<?=$row[username] ?>'" style="width: 20px;"></img>	            
            <?php
            }
        ...
        <div class="content_wrapper content_mb_60">
            ...
            <div class="col_34 no_margin_right">
                <h3>Post Concert</h3>
                <a href="postconcert.php" class="more">Start</a>
            </div>
            </form>
            <div class="clear"></div>
        </div>
